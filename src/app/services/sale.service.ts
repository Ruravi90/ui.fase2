...
export class SaleService {
    ...
    getSalesUserDay(): Observable<Sale[]> {
      return this.http.get<Sale[]>(this.url + '/user/' + this.currentUser.id).pipe(map((r:any)=> r));
    }
    getCuteSales(): Observable<Sale[]> {
        return this.http.post<Sale[]>(this.url + '/cute_now', { user_id:  this.currentUser.id }).pipe(map((r:any)=> r));
    }
